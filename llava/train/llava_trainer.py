...
# Replace the line: from transformers.trainer_pt_utils import AcceleratorConfig
try:
    from transformers.trainer_pt_utils import AcceleratorConfig
except ImportError:
    # This class doesn't exist in transformers 4.37.2, so we stub it
...
class LLaVATrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        # 1. Load the linguistic embeddings generated by simularity.py
        # Ensure ling_emb_path is passed in through training arguments
        if hasattr(self.args, 'ling_emb_path') and os.path.exists(self.args.ling_emb_path):
            rank0_print(f"Loading linguistic embeddings from {self.args.ling_emb_path}")
            self.ling_embeddings = torch.load(self.args.ling_emb_path).to(self.args.device)
        else:
            self.ling_embeddings = None

    def compute_loss(self, model, inputs, return_outputs=False):
        """
        Modified compute_loss to implement Language-Aware Hardness Weighting (Eq 2 & 3)
        """
        # 1. Extract inputs
        qry_inputs = inputs['qry_inputs']
        pos_inputs = inputs['pos_inputs']

        # 2. Get embeddings
        query_embeds = model.encode_multimodal_embeddings(**qry_inputs)
        pos_embeds = model.encode_multimodal_embeddings(**pos_inputs)
        ...
        logit_scale = getattr(model.config, "logit_scale", 50.0) 
        sim_embed = torch.matmul(query_embeds, pos_embeds.T) * logit_scale
        
        # --- FIX 1: UPCAST TO FP32 FOR MATH STABILITY ---
        # Prevents BFloat16 from rounding small negative penalties to 0
        sim_embed = sim_embed.to(torch.float32)

        # 4. Calculate Linguistic Similarity
        if self.ling_embeddings is not None and 'indices' in inputs:
            batch_indices = inputs['indices'] 
            batch_ling = self.ling_embeddings[batch_indices]
            sim_ling = torch.matmul(batch_ling, batch_ling.T).to(torch.float32)
        else:
            sim_ling = torch.zeros_like(sim_embed, dtype=torch.float32)

        # 5. Combined Hardness Score
        alpha = getattr(self.args, "alpha", 9.0)
        beta = getattr(self.args, "beta", 1.0)
        hardness_score = alpha * sim_embed + beta * sim_ling

        # 6. Hardness Weighting
        weights = torch.exp(hardness_score)
        mask = torch.eye(weights.size(0), device=weights.device, dtype=weights.dtype)
        weights = weights * (1 - mask) + mask
        weights = torch.clamp(weights, min=1e-6)

        # 7. Weighted Contrastive Loss 
        # --- FIX 2: LOGSUMEXP TRICK ---
        # Stays in log-space to completely avoid calculating 1.0 / 1.0
        row_max, _ = sim_embed.max(dim=1, keepdim=True)
        exp_sim = torch.exp(sim_embed - row_max)
        weighted_exp_sim = weights * exp_sim
        
        log_denom = torch.log(weighted_exp_sim.sum(dim=1).clamp(min=1e-12)) + row_max.squeeze(1)
        log_numer = torch.diag(sim_embed) 
        
        loss = (log_denom - log_numer).mean()

        return (loss, {"logits": sim_embed}) if return_outputs else loss
